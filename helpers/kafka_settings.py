"""Based on Heroku's kafka_helper but without the built-in requirement for python-kafka"""
...
def heroku_kafka_name(str):
    return "{0}{1}".format(os.environ.get('KAFKA_PREFIX'), str)


def get_asyncio_kafka_connector(loop, topic_name='first_topic', group_id_name='test'):
    return AIOKafkaConsumer(
        heroku_kafka_name(topic_name),
        group_id=heroku_kafka_name(group_id_name),
        loop=loop,
